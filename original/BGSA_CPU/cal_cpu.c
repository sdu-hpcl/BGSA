...
void
cpu_cal_align_score(char *content, cpu_read_t *preprocess_reads, cpu_write_t *align_results, int ref_len, int ref_count,
...
#pragma omp parallel for default(none) num_threads(cpu_threads) \
	...
	for (i = ref_start; i < ref_end; i++) {
		for (j = 0; j < loop_num; j++) {
			...
			align_cpu(ref_itr, read_itr, ref_len, read_len, word_num, tmp_read_num,
...
void cpu_cal(void *args) {
	...
	cpu_cal_align_score(ref_content, read_ptr, result_ptr, ref_len, ref_count, read_len, read_count, cpu_ref_start,
						cpu_ref_end, word_num, chunk_read_num, dvdh_bit_mem);
...
void cal_on_cpu() {
	...
	if (full_bits) {
		cpu_word_num = (read_len + CPU_WORD_SIZE - 1) / CPU_WORD_SIZE;
	} else {
