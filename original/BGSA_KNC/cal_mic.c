...
__ONMIC__ mic_read_t *preprocess_reads_a;
__ONMIC__ mic_read_t *preprocess_reads_b;
__ONMIC__ mic_read_t *read_ptr;

__ONMIC__ mic_write_t *align_results_a;
__ONMIC__ mic_write_t *align_results_b;
__ONMIC__ mic_write_t *result_ptr;


__ONMIC__ seq_t read_seq_a;
__ONMIC__ seq_t read_seq_b;
__ONMIC__ seq_t *read_seq;
__ONMIC__ seq_t ref_seq;

__ONMIC__ __m512i *dvdh_bit_mem;
...
__ONMIC__ void
mic_cal_align_score(char *content, mic_read_t *preprocess_reads, mic_write_t *align_results, int ref_len, int ref_count,
					int read_len, int read_count, int ref_start, int ref_end, int word_num, int chunk_read_num,
...
#pragma omp parallel for default(none) num_threads(mic_threads)\
	...
	for (i = ref_start; i < ref_end; i++) {
		for (j = 0; j < loop_num; j++) {
			...
			align_mic(ref_itr, read_itr, ref_len, read_len, word_num, tmp_read_num,
					  align_index_start + j * chunk_read_num, align_results, dvdh_bit_mem);
		}
	}
	//printf("111\n");
...
#pragma offload target(mic:mic_index) \
    ...
    in(mic_threads)
	{
		...
		mic_cal_align_score(ref_content, read_ptr, result_ptr, ref_len, ref_count, read_len, read_count, mic_ref_start,
							mic_ref_end, word_num, chunk_read_num, dvdh_bit_mem);
...
#pragma omp parallel for num_threads(mic_device_number) default(none)\
            private(i) shared(mic_device_number, preprocess_reads_a, up_signal_a, word_num, device_read_counts_a)
		for (i = 0; i < mic_device_number; i++) {
...
#pragma  omp parallel for num_threads(total_device_number) default(none) \
                ...
                shared(mic_args, total_device_number, mic_device_number, read_seq, read_ptr, result_ptr, ref_start, ref_end, word_num, sse_result_ptr, device_read_counts, cal_output_info )
			for (i = 0; i < total_device_number; i++) {
