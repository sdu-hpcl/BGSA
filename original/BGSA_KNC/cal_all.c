...
void sse_cal_all(void *args) {
	...
	GET_TIME(time_start);
	sse_cal_align_score(ref_content, read_ptr, result_ptr, ref_len, ref_count, read_len, read_count, sse_ref_start,
						sse_ref_end, word_num, chunk_read_num, dvdh_bit_mem);
	GET_TIME(time_end);
