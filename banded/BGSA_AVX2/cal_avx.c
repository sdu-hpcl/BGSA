    ...
    #pragma omp parallel for default(none) num_threads(cpu_threads) \
        ...
        for(i = ref_start; i < ref_end; i++) {
            for(j = 0; j < loop_num; j++) {
                ...
                read_itr = & preprocess_reads[CHAR_NUM * AVX_V_NUM * j * word_num * chunk_read_num];
                align_avx(ref_itr, read_itr, ref_len, read_len, word_num, tmp_read_num, align_index_start + j * chunk_read_num, align_results, dvdh_bit_mem);
            }
        }
}

void avx_cal(void * args) {

    avx_cal_t * cal_params = (avx_cal_t *) args;
    ...
    avx_read_t  * read_ptr = & (cal_params->read_ptr[read_ptr_offset]);
    avx_write_t * result_ptr = & (cal_params->result_ptr[result_ptr_offset]);
    __m256i * dvdh_bit_mem = cal_params->dvdh_bit_mem;
    ...
    int     avx_ref_start = cal_params->avx_ref_start;
    int     avx_ref_end = cal_params->avx_ref_end;
    ...
    GET_TIME(time_start);
    avx_cal_align_score(ref_content, read_ptr, result_ptr, ref_len, ref_count, read_len, read_count, avx_ref_start, avx_ref_end, word_num, chunk_read_num, dvdh_bit_mem);
    GET_TIME(time_end);
...
void cal_on_avx() {
    ...
    int64_t avx_align_result_size;

    avx_cal_t   avx_arg;
    avx_cal_t * tmp_avx_arg;
    ...
    /*if(full_bits) {
        avx_word_num = (read_len + AVX_WORD_SIZE - 1) / AVX_WORD_SIZE;
    } else {
        avx_word_num = (read_len + AVX_WORD_SIZE - 2) / (AVX_WORD_SIZE - 1);
    } */

    int h_threshold = threshold + read_len - ref_len;
    avx_word_num = (read_len - h_threshold + AVX_WORD_SIZE - 1) / AVX_WORD_SIZE + 1;
    //avx_word_num -= 1;
    chunk_read_num = (max_length + read_len - 1) / read_len;
    //printf("chunk_read_num is %d\n", chunk_read_num);
    avx_preprocess_read_size = sizeof(avx_read_t) * avx_word_num * CHAR_NUM * read_bucket_count;
    avx_preprocess_reads_a = (avx_read_t *) malloc_mem(avx_preprocess_read_size);
    avx_preprocess_reads_b = (avx_read_t *) malloc_mem(avx_preprocess_read_size);
    avx_align_result_size = sizeof(avx_write_t) * ref_bucket_count * read_bucket_count;
    avx_align_results_a = (avx_write_t *)malloc_mem(avx_align_result_size);
    avx_align_results_b = (avx_write_t *)malloc_mem(avx_align_result_size);
    avx_read_ptr = avx_preprocess_reads_a;
    avx_result_ptr = avx_align_results_a;
    avx_dvdh_size = avx_word_num * cpu_threads * dvdh_len;
    avx_dvdh_bit_mem = (__m256i *)malloc_mem(sizeof(__m256i) * avx_dvdh_size);

    result_bucket_counts[0] = read_seq_a.count;

    GET_TIME(process_start);
    GET_TIME(mem_start);
    memset(avx_preprocess_reads_a, 0, avx_preprocess_read_size);
    avx_handle_reads(& read_seq_a, avx_preprocess_reads_a, avx_word_num, 0, read_seq_a.count);
    GET_TIME(mem_end);
    ...
    in_arg.avx_preprocess_read_size = avx_preprocess_read_size;
    in_arg.bucket_size = read_actual_size;
    in_arg.bucket_num = read_bucket_num;
    in_arg.avx_word_num = avx_word_num;

    output_thread_arg  out_arg;
    out_arg.fp = fp_result;
    out_arg.avx_result_count = & total_result_count;
    ...
    in_arg.avx_preprocess_reads_a = avx_preprocess_reads_a;
    in_arg.avx_preprocess_reads_b = avx_preprocess_reads_b;
    ...
    out_arg.avx_align_results_a = avx_align_results_a;
    out_arg.avx_align_results_b = avx_align_results_b;

    pthread_create(&(input_info.thread_id), NULL, input_task_avx, & in_arg);
    pthread_create(&(output_info.thread_id), NULL, output_task_avx, & out_arg);
    ...
    while(1) {
        ...
        if(cal_input_info.buffer_flag) {
            ...
            avx_read_ptr = avx_preprocess_reads_b;
        ...
        tmp_avx_arg = & avx_arg;
        tmp_avx_arg->ref_seq =  & ref_seq;
        tmp_avx_arg->read_seq = read_seq;
        tmp_avx_arg->dvdh_bit_mem = avx_dvdh_bit_mem;
        tmp_avx_arg->word_num = avx_word_num;
        tmp_avx_arg->chunk_read_num = chunk_read_num;
        tmp_avx_arg->cal_total_time = & cal_total_times;
        tmp_avx_arg->read_ptr = avx_read_ptr;
        ...
        for(ref_bucket_index = 0; ref_bucket_index < ref_bucket_num; ref_bucket_index++) {
            ...
            if(cal_output_info.buffer_flag) {
                ...
                avx_result_ptr = avx_align_results_b;
            }
            ...
            tmp_avx_arg = & avx_arg;
            tmp_avx_arg->avx_ref_start = ref_start;
            tmp_avx_arg->avx_ref_end = ref_end;
            tmp_avx_arg->read_ptr_offset =  0;
            tmp_avx_arg->result_ptr_offset = 0 ;
            tmp_avx_arg->result_ptr = avx_result_ptr;
            tmp_avx_arg->read_count = read_seq->count;
            avx_cal(tmp_avx_arg);
    ...
    //printf("%p\n", avx_preprocess_reads_b);
    free_mem(avx_preprocess_reads_a);
    free_mem(avx_preprocess_reads_b);

    free_mem(avx_align_results_a);
    free_mem(avx_align_results_b);
    free_mem(avx_dvdh_bit_mem);
