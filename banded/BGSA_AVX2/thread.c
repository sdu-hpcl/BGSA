...
void * input_task_avx(void * args) {
    ...
    avx_read_t  * avx_preprocess_reads_a = in_arg->avx_preprocess_reads_a;
    avx_read_t  * avx_preprocess_reads_b = in_arg->avx_preprocess_reads_b;
    seq_t       * read_seq_a = in_arg->read_seq_a;
    seq_t       * read_seq_b = in_arg->read_seq_b;
    seq_t       * seq;
    avx_read_t  * avx_p_reads;

    int64_t total_size = in_arg->total_size;
    int     bucket_num = in_arg->bucket_num;
    int     avx_word_num = in_arg->avx_word_num;
    ...
    int64_t avx_preprocess_read_size = in_arg->avx_preprocess_read_size;
    ...
    while(1) {
        ...
        if(input_info->buffer_flag) {
            ...
            seq = read_seq_a;
            avx_p_reads = avx_preprocess_reads_a;
            device_read_counts = device_read_counts_a;
        } else {
            remain_size -= read_seq_a->size;
            seq = read_seq_b;
            avx_p_reads = avx_preprocess_reads_b;
        ...
        GET_TIME(mem_start);
        memset(avx_p_reads, 0, avx_preprocess_read_size);
        avx_handle_reads(seq, avx_p_reads, avx_word_num, 0, seq->count);
        GET_TIME(mem_end);
...
void * output_task_avx(void * args) {
     ...
     FILE    * fp = out_arg->fp;
     int64_t * avx_result_count = out_arg->avx_result_count;

     thread_info * output_info = out_arg->output_info;
     thread_info * cal_output_info = out_arg->cal_output_info;
     avx_write_t * avx_align_results_a = out_arg->avx_align_results_a;
     avx_write_t * avx_align_results_b = out_arg->avx_align_results_b;
     ...
     while(1) {
        ...
        if(output_info->buffer_flag) {
            fwrite(avx_align_results_a, sizeof(avx_write_t), * avx_result_count, fp);
            fflush(fp);

        } else {
            fwrite(avx_align_results_b, sizeof(avx_write_t), * avx_result_count, fp);
            fflush(fp);
        }
        GET_TIME(write_end);
