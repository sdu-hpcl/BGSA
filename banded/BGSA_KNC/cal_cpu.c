...
void cpu_cal_align_score(char * content, cpu_read_t * preprocess_reads, cpu_write_t * align_results, int ref_len, int ref_count, int read_len, int read_count, int ref_start, int ref_end, int word_num, int chunk_read_num, uint64_t * dvdh_bit_mem) {
    ...
    #pragma omp parallel for default(none) num_threads(cpu_threads) \
        private( j, read_itr, ref_itr, align_index_start, tmp_read_num)  \
        shared( i,ref_len,read_len,ref_start, ref_end, content, align_results, preprocess_reads, loop_num, loop_num_temp, word_num, dvdh_bit_mem, chunk_read_num, rest_read_num) \
        schedule(guided)
...
void cal_on_cpu() {
    ...
    cpu_word_num = (read_len - h_threshold + CPU_WORD_SIZE - 1) / CPU_WORD_SIZE + 1;
    ...
    cpu_dvdh_size = cpu_threads * dvdh_len;
